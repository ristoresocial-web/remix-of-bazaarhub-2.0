...
const ChatbotPanel: React.FC<Props> = ({ onClose }) => {
  ...
  return (
    <>
      ...
      />
      <div
        className="fixed bottom-4 right-4 z-50 flex h-[520px] w-[380px] max-w-[calc(100vw-2rem)] max-h-[calc(100vh-2rem)] flex-col rounded-card border border-border bg-card shadow-lg overflow-hidden animate-in fade-in slide-in-from-bottom-2 duration-200"
        role="dialog"
        aria-label="BazaarBot chat"
      >
        {/* Header */}
        <div className="flex items-center justify-between bg-primary px-4 py-3">
          ...
          <button
            onClick={onClose}
            className="rounded-full p-1 text-primary-foreground/80 hover:bg-primary-foreground/15 hover:text-primary-foreground transition-all duration-200"
